...
func main() {
	input, err := os.ReadFile("./cmd/day8a/sample.txt")
	if err != nil {
		log.Fatal(err)
	}
	...
	for _, locs := range locationsByFrequency {
		for _, locationA := range locs {
			for _, locationB := range locs {
				...
				if uap == upper || uap == lower {
					continue
				}
				if uap.Y >= 0 && uap.Y <= maxY && uap.X >= 0 && uap.X <= maxX {
				...
				if lap == upper || lap == lower {
					continue
				}
				if lap.Y >= 0 && lap.Y <= maxY && lap.X >= 0 && lap.X <= maxX {
